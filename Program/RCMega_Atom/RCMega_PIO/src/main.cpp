...
const char *ssid = "RCMega";    // The name of the Wi-Fi network that will be created
const char *pass = "123456789";   // The password required to connect to it, leave blank for an open networ
...
void WSEventHandler(uint8_t num, WStype_t type, uint8_t * payload, size_t length)
{
  ...
  if (type == WStype_TEXT)
  {
    ...
    if(cmd == String("S"))
    {
      int valToWrite = (int)(90.0f - (30 * param));
...
void setup()
{
  ...
  Serial.printf("  - Creating access point\n");
  WiFi.mode(WIFI_AP);
